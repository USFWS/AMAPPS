...
ind = datasets %>% filter(parent_id %in% c(8,9))
obs_doe_midA = obs %>% filter(dataset_id %in% ind)
# ------------------ #
...
datasets = transformDataset(datasets) %>% 
  dplyr::select(-data_url, -report, -data_citation, -publications, 
                -publication_url, -publication_DOI, -id,
                -dataset_summary, -dataset_processing, -dataset_quality) %>%
  left_join(., lit, by="dataset_id") %>% 
  left_join(., summaries, by="dataset_id")

# export
write.csv(obs_midA,"Z:/seabird_database/data_sent/AGilbert_DE_DOEMidA_Dec2018/hidef_observations.csv")
write.csv(obs_bb,"Z:/seabird_database/data_sent/AGilbert_DE_DOEMidA_Dec2018/DE_observations.csv")
write.csv(cc.datasets,"Z:/seabird_database/data_sent/AGilbert_DE_DOEMidA_Dec2018/datasets.csv")
# ------------- #
