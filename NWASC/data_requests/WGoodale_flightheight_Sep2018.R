...
# pull out obs where NULL is written
all.data = filter(all.data, !flight_height_tx %in% c("","NULL","Not Possible","unk"))
...
# export 
# -------------- #
write.csv(datasets, "//ifw-hqfs1/MB SeaDuck/seabird_database/data_sent/WGoodale_flightheight_Sept2018/datasets.csv",
          row.names = F)
write.csv(all.data, "//ifw-hqfs1/MB SeaDuck/seabird_database/data_sent/WGoodale_flightheight_Sept2018/observations.csv",
          row.names = F)

# pull species look up table too for reference
db <- dbConnect(odbc::odbc(), driver='SQL Server',server='ifw-dbcsqlcl1', database='NWASC')
species_codes = dbGetQuery(db,"select * from lu_species")
dbDisconnect(db)
species_codes = dplyr::select(species_codes,-order,-family,-subfamily)
write.csv(all.data, "//ifw-hqfs1/MB SeaDuck/seabird_database/data_sent/WGoodale_flightheight_Sept2018/species_codes.csv",
          row.names = F)
# # -------------- #
